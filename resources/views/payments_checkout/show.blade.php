    ...
    <div class="flex items-center justify-center min-h-[70vh]">
        <div class="w-full max-w-2xl bg-white/90 backdrop-blur rounded-2xl shadow-xl p-8 border border-slate-200">
            ...
            <div class="text-center mb-6">
                <h1 class="text-2xl font-bold text-slate-700 mt-1">Detalles de la factura - prueba checkout</h1>
            </div>
            ...
            <div class="space-y-3 mb-6 mt-3">
                <div class="flex justify-between border-b pb-2">
                    <span class="font-medium text-slate-600">Numero de factura:</span>
                    <span class="text-slate-800">{{ $invoice->numero }}</span>
                </div>

                <div class="flex justify-between border-b pb-2">
                    <span class="font-medium text-slate-600">Numero de identificacion:</span>
                    <span class="text-slate-800">{{ $invoice->codigo }}</span>
                </div>

                <div class="flex justify-between border-b pb-2">
                    <span class="font-medium text-slate-600">Fecha limite:</span>
            ...
            @elseif ($vencida)
                <div class="mb-6 p-3 rounded-lg bg-red-50 text-red-700 text-sm border border-red-200 text-center">
                    Esta factura esta vencida. Ya no se encuentra habilitada para pago en linea.
                </div>
            @else
                <form method="POST" action="{{ route('pago.checkout.link', $invoice->refpago) }}" class="mb-6">
                    @csrf
                    <button type="submit"
                        class="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-semibold py-3 rounded-lg shadow-md transition duration-200">
                        Pagar ahora en prueba checkout
                    </button>
                </form>
            @endif

            <div class="text-center">
                <a href="{{ route('pago.checkout.search.form') }}" class="text-sky-600 hover:text-sky-700 text-sm font-medium">
