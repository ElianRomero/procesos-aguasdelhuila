@if (auth()->check() && optional(auth()->user()->proponente)->id)
      ...
      <ul class="divide-y">
        @foreach ($ultimas as $n)
          @php
            ...
            $mes = ['01'=>'ene','02'=>'feb','03'=>'mar','04'=>'abr','05'=>'may','06'=>'jun','07'=>'jul','08'=>'ago','09'=>'sep','10'=>'oct','11'=>'nov','12'=>'dic'][$dt->format('m')] ?? '';
          ...
          <li class="px-2 py-1.5 transition-colors {{ $esNoLeida ? 'bg-green-50 hover:bg-green-100' : '' }}"
              data-noticia-id="{{ $n->id }}">
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0">
                {{-- 1) Título (primero, truncado a 14) --}}
                <div class="font-semibold text-[12px] js-title {{ $esNoLeida ? 'text-green-800' : '' }}">
              ...
              <span class="js-type-badge shrink-0 inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full border
                           {{ $esNoLeida ? 'bg-green-100 text-green-800 border-green-200' : 'bg-gray-50 text-gray-700 border-gray-200' }}">
                @if($esNoLeida)
                  <span class="js-unread-dot inline-block w-1.5 h-1.5 rounded-full bg-green-500"></span>
                @endif
                {{ $n->tipo }}
              </span>
            </div>
            ...
            <div class="mt-1">
              <a href="{{ $urlShow }}"
                 class="text-[10px] px-1.5 py-0.5 rounded border hover:bg-gray-50 js-mark-leida"
  ...
  <script>
    (function () {
      ...
      function updateUI(anchorEl) {
        // Quitar estilos de "no leída" en el item
        const li = anchorEl.closest('li[data-noticia-id]');
        if (!li) return;

        li.classList.remove('bg-green-50', 'hover:bg-green-100');
        const title = li.querySelector('.js-title');
        title?.classList.remove('text-green-800');

        // Cambiar badge de tipo a neutro y quitar dot
        const badge = li.querySelector('.js-type-badge');
        if (badge) {
          badge.classList.remove('bg-green-100','text-green-800','border-green-200');
          badge.classList.add('bg-gray-50','text-gray-700','border-gray-200');
          const dot = badge.querySelector('.js-unread-dot');
          dot?.remove();
        }

        // Decrementar contador rojo (si existe)
      ...
      document.querySelectorAll('.js-mark-leida').forEach(a => {
        a.addEventListener('click', function () {
          ...
          updateUI(this);    // optimismo visual
          markRead(url);     // disparo real al backend
        }, { passive: true });
      });
    })();
  </script>
