...
<script>
  // Mostrar el motivo real si falla el AJAX:
  ...
  const tabla = $('#tabla')
    .on('error.dt', function (e, settings, techNote, message) {
      ...
      if (xhr) console.error('Response:', xhr.status, xhr.responseText);
      alert('Error cargando datos (' + (xhr ? xhr.status : 'n/a') + '). Revisa consola (F12).');
    })
    .DataTable({
      serverSide: true,
      processing: true,
      deferRender: true,
      pageLength: 10,
      order: [[1, 'desc']], // fecha DESC por defecto
      autoWidth: false,
      searching: false, // ← evitamos que mande search[value] por defecto
      ajax: {
        // Misma origin del iframe
        url: "/api/procesos",
        type: "GET",
        data: function (d) {
          ...
          d.mes              = $('#mes').val()              || '';
          d.anio             = $('#anio').val()             || '';
          d.q                = $('#q').val()                || ''; // ← SIN corchetes (evita WAF)
        }
      },
      columns: [
        { data: 'codigo', title: 'Código' },
        {
          ...
          render: function (v) {
            ...
            try { return new Date(v).toISOString().slice(0,10); } catch { return s.slice(0,10); }
          }
        },
        { data: 'objeto', title: 'Objeto' },
        { data: 'tipo_proceso', title: 'Tipo proceso' },
        { data: 'estado_contrato', title: 'Estado' },
        { data: 'tipo_contrato', title: 'Tipo contrato' },
        {
          ...
          render: function (v) {
            if (v == null || v === '') return '';
            const n = Number(v);
            return isNaN(n) ? v : n.toLocaleString('es-CO', { style: 'currency', currency: 'COP' });
          }
        },
        {
          data: null,
  ...
  // Enter en búsqueda
  $('#q').on('keyup', (e) => { if (e.key === 'Enter') $('#btnFiltrar').click(); });

  // Autoaltura para iframe
  function postHeight() {
    const h = document.body.scrollHeight;
    parent.postMessage({ type: 'aguas:height', px: h }, '*');
  }
